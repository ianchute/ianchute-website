...
The `home-routes` handler in the `website.routes.home` namespace
...
The `home-page` function will in turn call the `website.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/website/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `website.handler` namespace:
...
* Let `mount` know to start the database connection by `require`-ing `website.db.core` in some other namespace.
